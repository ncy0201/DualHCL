...
from utils import get_adj, to_word2vec_format
import argparse

def parse_args():
    parser = argparse.ArgumentParser(description="DeepWalk")
    parser.add_argument('--s_path', default='data/douban/online.txt')
    parser.add_argument('--t_path', default='data/douban/offline.txt')
...
if __name__ == '__main__':
    args = parse_args()
    s_adj = get_adj(args.s_path)
    t_adj = get_adj(args.t_path)
    
    ### save initial embedding from random walk for training

    for i in range(args.N):
        print('Iteration: ', i)
        s_embedding = node2vec(s_adj, P=1, Q=1, WINDOW_SIZE=10, NUM_WALKS=10, WALK_LENGTH=80, DIMENSIONS=args.dim, \
			WORKERS=8, ITER=5, verbose=1)
        t_embedding = node2vec(t_adj, P=1, Q=1, WINDOW_SIZE=10, NUM_WALKS=10, WALK_LENGTH=80, DIMENSIONS=args.dim, \
			WORKERS=8, ITER=5, verbose=1)     
        
        to_word2vec_format(s_embedding, args.out_path, f'source_emb_{args.dim}_{i}')
        to_word2vec_format(t_embedding, args.out_path, f'target_emb_{args.dim}_{i}')
